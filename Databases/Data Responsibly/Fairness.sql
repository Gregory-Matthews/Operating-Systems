-- CS 500: Homework 6
-- Mining Fair Classification Rules
-- By Greg Matthews
-- 3/7/18
...
insert into Impact_Ratio (attr1, val1, attr2, val2, ratio)
	...
	and pos.supp / (pos.supp+neg.supp) >= (select conf from Q)
);

-- PART 3: EliftRatio (S+ X => C) = support(S+ X => C) / support (X => C)
insert into Elift_Ratio (attr1, val1, attr2, val2, attr3, val3, ratio)
	(select R3.attr1, R3.val1, R3.attr2, R3.val2, R3.attr3, R3.val3, (pos.supp/F2.supp) as ratio
	from R3, F3 as pos, F2
	where R3.attr3 = 'vdecile' and R3.val3 = 1
	and R3.attr2 = 'race' and R3.val2 = 1

	
	and R3.attr3 = pos.attr3
	and R3.val3 = pos.val3
	and R3.attr2 = pos.attr2
	and R3.val2 = pos.val2
	and R3.attr1 = pos.attr1
	and R3.val1 = pos.val1
	
	
	and R3.attr1 = F2.attr1
	and R3.val1 = F2.val1
	and R3.attr3 = F2.attr2
	and R3.val3 = F2.val2
	and R3.supp >=  (select conf from Q)
	and (pos.supp+F2.supp) / R3.supp >= (select conf from Q)
	);
